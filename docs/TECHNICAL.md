...
3. **Refresh Steam**: Because Steam sometimes caches your library UI, you may need to **Restart your Steam Deck** (or switch to Desktop Mode and back to Gaming Mode) for the newly created shortcut to physically appear in your Non-Steam games list.
4. **Proton Configuration**: Before launching the game for the first time, select the new Dimensions Recompiled shortcut, go to its **Properties -> Compatibility**, check the box to force the use of a specific Steam Play compatibility tool, and select **Proton Experimental** or **Proton GE**.
5. **Auto-Patching Magic**: When the plugin created your shortcut, it seamlessly patched your game's `legodimensions.toml` configuration file. It converted the installer's hardcoded Windows paths (e.g., `C:\Games\...`) into universal Linux/Proton paths mapped to the `Z:\` drive. This completely eliminates the notorious `--game_data_root does not exist` crash!
6. **Save Migration**: If migrating a PS4 save via DimensionsSaveConverter, launch the Recompiled game once to create a blank save (Slot 1). Then point the converter's output directly into the newly generated `00000001` folder inside your `content` directory.
7. **Playing**: Simply launch the "Dimensions Recompiled" shortcut directly from your Steam library in Gaming Mode. The Decky plugin handles all custom tag spawning, LED sync, and file abstraction natively over Proton without any extra configuration needed!